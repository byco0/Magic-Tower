...
class Monster(GeneralSquare):
    ...
    def draw_popup(self, player, soundfile):
        surf = pygame.Surface((POPUP_X, POPUP_Y))
        surf.fill(GREY)
        pygame.draw.rect(surf, ORANGE, [0, 0, POPUP_X, POPUP_Y], 6)
        font = pygame.font.Font(None, int(AVATAR*2/7))
        ...
        pygame.draw.rect(surf, ORANGE, [mimg_X-10, mimg_Y-10, AVATAR+20, AVATAR+20], 3)
        ...
        pygame.draw.rect(surf, ORANGE, [pimg_X-10, pimg_Y-10, AVATAR+20, AVATAR+20], 3)
        playername = font.render('Player', True, WHITE)
        surf.blit(playername, (pimg_X+AVATAR/2-playername.get_width()/2,pimg_Y+AVATAR+40))

        # Add sound effects
        effect = pygame.mixer.Sound(os.path.join(SOUND_DIR, soundfile))
        ...
        while self.HP > 0:
            ...
            # Draw VS
            font2 = pygame.font.Font(None, AVATAR//2)
            VS_text = font2.render('VS', True, WHITE)
            surf.blit(VS_text, (POPUP_X/2 - 60, POPUP_Y/2 - 55))
            # Monster state
